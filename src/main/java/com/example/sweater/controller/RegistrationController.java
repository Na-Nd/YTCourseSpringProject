...
public class RegistrationController {
    ...
    @Value("${recaptcha.secret}") // Секретный ключ из application.properties
    ...
    public String addUser(@RequestParam("password2") String passwordConfirm, @RequestParam("g-recaptcha-response")String captchaResponse, @Valid User user, BindingResult bindingResult, Model model){
        ...
        if(!response.isSuccess()){ // Если ответ неуспешный
            model.addAttribute("captchaError", "Fill captcha");
        }

        boolean isConfirmEmpty = StringUtils.isEmpty(passwordConfirm);

        if(isConfirmEmpty){ // Если passwordConfirm не пустой
            model.addAttribute("password2Error", "Password confirmation cannot be empty");
        }

        // Проверим, сходятся ли два пароля, которые пользователь ввел в форму при регистрации
